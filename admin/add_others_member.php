...
{ 
  ...
  if(in_array($_FILES["file"]["type"],$allowedFileType)){
        ...
        $query = "INSERT INTO `tbl_others_member` (`loksabha`, `vidhansabha`, `mandal`, `panchayat`, `booth`, `name`, `fathers_name`, `age`, `caste`, `mobile`, `news_paper_channel`, `address`, `dob`) VALUES";
        $sub_query = "";
        ...
        foreach ($Reader as $Row){     
            if($counter >= $skipRows){
                ...
                if(isset($Row[$skipColumns+10])) {
                    $news_paper_channel = $Row[$skipColumns+10];
                }
                $address = ''; 
                if(isset($Row[$skipColumns+11])) {
                    $address = $Row[$skipColumns+11];
                }
                $dob = NULL; 
                if(isset($Row[$skipColumns+12]) && $Row[$skipColumns+12] != '') {
                    $dob = $Row[$skipColumns+12];
                    $dob = strtotime($dob);
                    $dob = date('d-m-Y',$dob);
                } 
                
                if($loksabha){
                    $sub_query = $sub_query." ('$loksabha', '$vidhansabha', '$mandal', '$panchayat', '$booth', '$name', '$fathers_name', '$age', '$caste', '$mobile', '$news_paper_channel', '$address', '$dob')\n,";
                }
            }
            $counter++;
        }
